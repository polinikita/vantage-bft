...
/// `TimeoutAccept::author`). `CutCertificate` carries no single claimed sender -- only
/// an aggregate `votes: Vec<PublicKey>` quorum list, and any party may be the one to
/// first assemble and relay it (unlike a leader-authored message, there is no "expected
/// broadcaster" to attribute it to positionally either); `CutCertificate::verify`
/// already independently checks every one of those votes against committee membership
/// (and rejects a repeated voter) before `CutEngine` ever acts on it, so it maps to
/// `None` exactly like Vantage's own quorum-certified/no-single-sender variants
/// (`Propose`/`ControlInit`/`ControlServe`). `TimerFired` is locally generated by this
/// node's own armed timer, never received over the wire, so it also maps to `None`.
/// `CutFetch` carries an explicit requester (mirrors `vantage::node::Inbound::
/// ControlFetch`), so it maps to `Some`. `CutServe` carries a `CutProposal` whose
/// `proposer` field names the ORIGINAL round leader, not the relaying/serving party
/// -- there is no sender claim on this variant to bind, exactly like Vantage's own
/// `ControlServe`, so it maps to `None` too (gated downstream instead, by
/// `CutEngine::on_cut_serve`'s own `pending_cut_fetch` check).
impl DeclaredSender for engine::Inbound {
    fn declared_sender(&self) -> Option<PublicKey> {
        match self {
            ...
            engine::Inbound::CutCertificate(_) => None,
...
pub enum Inbound {
    ...
    /// The six wire-received cut-consensus messages, forwarded to `CutEngine::handle`
    /// verbatim.
    Cut(engine::Inbound),
}
...
fn mac_candidate_sender(message: &PrimaryMessage) -> Option<PublicKey> {
    match message {
        ...
        PrimaryMessage::SimpleItCutCertificate(_) => None,
...
impl MessageHandler for SimpleItReceiverHandler {
    async fn dispatch(
        ...
        let inbound = match message {
            ...
            PrimaryMessage::SimpleItCutCertificate(c) => {
                Inbound::Cut(engine::Inbound::CutCertificate(c))
            }
...
pub struct SimpleItCore {
    ...
    /// commit a round via a `CutCertificate` plus a quorum of `Decide`s without ever
    /// having voted on (or gated via `gate_tips`/gate on) that round's own tips --
...
impl SimpleItCore {
            ...
            match effect {
                CutEffect::Broadcast(out) => {
                    let message = match out {
                        ...
                        CutOut::CutCertificate(c) => PrimaryMessage::SimpleItCutCertificate(c),
    ...
    /// NOT hold: `try_commit_round`/`emit_commit_to_committer` fire off a
    /// `CutCertificate` plus a quorum of `Decide`s, none of which requires this node
    /// to have voted on (hence `gate_tips`-gated on) that round's own tips, and
