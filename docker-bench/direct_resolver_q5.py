...
def load_run(
    ...
    if failed:
        if not failed <= {"run result captured", "open backlog drained"}:
    ...
    return {
        ...
        "record": {
            ...
            "correct_tps": float(throughput["committed_tps"]) if throughput else None,
            ...
            "median_node_cpu_cores": float(throughput["median_node_cpu_cores"]) if throughput else None,
            "maximum_node_wire_mbps": float(throughput["max_node_wire_mbps"]) if throughput else None,
        },
    }
...
def main(argv=None) -> None:
    ...
    aggregate = {
        key: (lambda vals: numeric_summary(vals) if vals else None)(
