#!/usr/bin/env python3
"""Two-arm transient-crash figure: per-second medians across repetitions for the baseline and early-refusal arms.

usage: crash_compare_plot.py BASE_ROOT LINK_ROOT OUT_DIR
Prints a JSON summary of the median curves.
"""
...
def main(base_root, link_root, out_dir):
    base, link = load(base_root), load(link_root)
    duration = min(r["duration"] for r in base + link)
    down = statistics.median(r["down"] for r in base + link); up = statistics.median(r["up"] for r in base + link)
    ...
    colors = {"base": "#245b8a", "link": "#c55a11"}; labels = {"base": "absolute timers only", "link": "early refusals"}
    summary = {}
    for name, reps in (("base", base), ("link", link)):
        tmed, _, _ = median_curve(reps, "tps", duration)
        lmed, llo, lhi = median_curve(reps, "lat", duration)
        top.plot(*zip(*tmed), color=colors[name], label=labels[name])
        bottom.plot(*zip(*lmed), color=colors[name], label=labels[name])
        bottom.fill_between([s for s, _ in llo], [v for _, v in llo], [v for _, v in lhi], color=colors[name], alpha=0.18, linewidth=0)
        summary[name] = {
            "reps": len(reps),
            "pre_p50_ms": statistics.median(window(lmed, 5, down - 2)),
            "crash_window_p50_ms": statistics.median(window(lmed, down + 5, up)),
            "crash_window_p90_ms": sorted(window(lmed, down + 5, up))[int(0.9 * (len(window(lmed, down + 5, up)) - 1))],
            ...
            "recovered_at_s": next((s for s, v in lmed if s > up and all(w < 1.1 * statistics.median(window(lmed, 5, down - 2)) for t, w in lmed if s <= t <= min(duration, s + 5))), None),
    ...
    for slot, (name, reps) in enumerate((("base", base), ("link", link))):
        _, _, lhi = median_curve(reps, "lat", duration)
        over = [(s, v) for s, v in lhi if v > y_cap]
        if over:
            s_max, v_max = max(over, key=lambda x: x[1])
            bottom.annotate(f"one {labels[name]} repetition: {v_max/1000:.1f} s (off scale)", (s_max, y_cap * 0.98),
                            xytext=(10, -16 - 13 * slot), textcoords="offset points", ha="left", va="top", fontsize=7,
                            color=colors[name], arrowprops={"arrowstyle": "-", "color": colors[name], "lw": 0.6})
    ...
    bottom.text((down + up) / 2, 0.96, f"{len(json.load(open(next(Path(base_root).glob('rep-*')) / 'data/chaos-timeline.json'))['victims'])} validators crashed",
                transform=bottom.get_xaxis_transform(), ha="center", va="top", fontsize=7.5, color="#444444",
