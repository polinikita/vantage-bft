...
def render_tc_script(
    ...
    # Put every destination class below one aggregate egress class. A finite
    # cap makes leader concentration observable locally; zero retains the
    # historical 10 Gbit/s ceiling used only to host the delay classes.
    egress_rate = f"{egress_mbps}mbit" if egress_mbps else "10gbit"
    lines.append('tc qdisc add dev "$IFACE" root handle 1: htb default 999')
    lines.append(
        f'tc class add dev "$IFACE" parent 1: classid 1:1 htb '
        f'rate {egress_rate} ceil {egress_rate} quantum 60000'
    )
    lines.append(
        f'tc class add dev "$IFACE" parent 1:1 classid 1:999 htb '
        f'rate {egress_rate} ceil {egress_rate} quantum 60000'
    )
    for j in range(n):
        if j == i:
...
def render_compose(
    ...
    for i in range(n):
        ...
        out += [
            ...
            f"      TX_COUNTED: \"{'false' if i in uncounted_load else 'true'}\"",
...
def write_manifest(
    ...
    load_rates = distribute_rate(args.rate, load_node_indices)
    uncounted_load = set(uncounted_load_indices)
    honest_offered_tps = sum(
        share for index, share in load_rates.items() if index not in uncounted_load
    )
    manifest = {
        "nodes": n,
        "crash": args.crash,
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    if args.leader_relay and args.withhold == 0:
        p.error("--leader-relay requires --withhold > 0")
    if args.leader_relay and args.withhold != fault_budget:
        p.error(f"--leader-relay requires exactly f={fault_budget} publishers")
